...
xe::Sound::Sound(const std::string_view &name, const std::string_view &path) :
		name(name),
		path(path),
		playing(false),
		count(0),
...
void xe::Sound::setGain(float gain) {
	if (!playing) {
...
void xe::destroyOnFinish(ga_Handle *handle, void *context) {
	xe::Sound *sound = (xe::Sound *) handle->sound;
	sound->count--;
...
void xe::loopOnFinish(ga_Handle *handle, void *context) {
