...
namespace xe { namespace internal {
	...
	PlatformWindowWin32::PlatformWindowWin32(VideoMode mode, const std::string_view &title, uint style) :
			handle(nullptr),
			callback(0),
			cursorVisible(true),
			lastCursor(LoadCursor(NULL, IDC_ARROW)),
			icon(nullptr),
	...
	void PlatformWindowWin32::setTitle(const std::string_view &title) {
		std::wstring output;
		output.reserve(title.size() + 1);
		utf32::toWide(title.begin(), title.end(), std::back_inserter(output), 0);
