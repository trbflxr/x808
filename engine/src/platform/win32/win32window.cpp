	...
	LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
		...
		switch (msg) {
			...
			case WM_SYSCOMMAND: {
				...
				break;
			case WM_SETFOCUS: focusCallback(window, true);
				break;

			case WM_KILLFOCUS: focusCallback(window, false);
				break;
			...
			case WM_SYSKEYDOWN:
			case WM_SYSKEYUP:
				keyCallback(inputManager, static_cast<int32>(lparam), static_cast<int32>(wparam), msg);
				break;

			case WM_MOUSEWHEEL:
				mouseWheelCallback(inputManager, GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam),
			...
			case WM_RBUTTONUP:
			case WM_MBUTTONDOWN:
			case WM_MBUTTONUP: mouseButtonCallback(inputManager, msg, GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam));
				break;
